...
class MCPToolAgent(Runnable):  # pylint: disable=redefined-builtin
    ...
    async def ainvoke(
        self, input: Any, config: Optional[RunnableConfig] = None, **kwargs: Any  # pylint: disable=redefined-builtin
    ) -> dict:
    ...
    def batch(
        self, inputs: List[Any], config: Optional[RunnableConfig] = None, **kwargs: Any  # pylint: disable=redefined-builtin
    ) -> List[dict]:
        raise NotImplementedError(
